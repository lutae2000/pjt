...
'use strict';
//alert("업데이트 디바이스 데이터 진행 fit.manger.js");
...
//	alert("업데이트 디바이스 fit.manger.js로딩");
    if ( ! item ) {
  //  	alert("item 업데이트 실패");
        console.trace();
...
*/
            ...
            sendSummary: function( callback ) {
                ...
                */
                ...
                debug.warn( "ljy Sync Send Summary - items ", items );
            //    alert("item획득");
                debug.log( "ljy SynchronizeManager items", items );

                _controller.execute("fit.sync.summary.send", { "items": items, "device": _deviceType }, {
                    showIndicator:false,
                    callback: function( event ) { 
                    	 debug.log( "ljy event", event.result );	
                        if ( event.error ) {
...
FitManager = Class({
    ...
    constructor: function() {
        ...
        var isUnpairedClick = false;

        return {
        	
        	setUnpairClick: function( clicked){
        		this.isUnpairedClick = clicked;
        	},
        	getUnpairClick: function(){
        		return isUnpairedClick;
        	},
            ...
            connect: function( callback, force ) {
                ...
                if ( ! M.fit.bluetooth.enabled() ) {
                    ...
                    //////////////////////////////////////////////////////////////////
                    	...
                    	M.execute("ex2PluginShowBandList");
                    }
                }
            },
            ...
            updateDashboardWithSampleData: function( result ) {
                debug.log( "FitManager - updateDashboardWithSampleData", self.userInfo().userKey() +" : " + result.user_profile_url );

            	_dashboard = result;

            	
            	var profileUrl = (result.user_profile_url == undefined)? "" : result.user_profile_url;
            ...
            dashboardData: function() {
                ...
                debug.warn( "sleepDashboardData_status", sleepStatus );
                //밴드에서 제대로 데이터를 가지고 오지 못할경우 수면상태가 unknown으로 뜬다  
                if(sleepStatus == "unknown"){
                	sleepStatus == "good"
                }
            ...
            sleepYesterdayDataByHTTP: function( callback ){
                ...
                _controller.execute("fit.sleep.yesterday", {}, {
                     ...
                     callback: function( event ) { 
						...
						//총 수면시간 설정
						$('#sleep_total').html(event.result.timeData.total.hour+'시간'+event.result.timeData.total.minute+'분');

						//선잠 설정
						$('#sleep_soso').html(event.result.timeData.soso.hour+'시간'+event.result.timeData.soso.minute+'분');
			            
				        //수면 상태 설정
						$('#sleep_state').html(event.result.status);
