...
Debug.type( (window.debug_type == "debug" ? "debug" : "console") );
...
Class.register = function( name, object, force ) {
	...
	if ( window["class_to_global"] === true ) {
		window[name] = object;
	}
};
		...
		return {
				...
				for ( var i=0; i<6; i++ ) {
					...
					$.each(_options.dayNames, function( idx, name ) {
						...
						if ( day == 0 ) {
							...
							dayHtml = '<td id="click_day" data-current-month data-action="day" '+disabledStr+' class="'+title+'" data-day="'+day+'">'+dayElFrontStr+day+dayElBackStr+'</td>';
						}
			...
			setupElements: function( element ) {				
				...
				for(i=0, len=Object.keys(_options.dayNames).length; i<len; i++) {
					var title = _options.dayNames[i];
					var week = '<th><span title='+title+' class='+title+'>'+title;
					week += '</span></th>';
					html += week;
				}
